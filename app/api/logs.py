...
from app.services.log_query_service import get_filtered_logs, get_log_summary, get_latest_logs
...
@router.get("/logs/live-feed", response_model=List[LogEventOut])
def live_feed_logs(limit: int = 25, db: Session = Depends(get_db)):
    return get_latest_logs(db, limit=limit)
